/*
** $Id: icon.c 11755 2009-06-13 11:54:22Z weiym $
**
...
HICON GUIAPI LoadIconFromFile (HDC hdc, const char* filename, int which)
{
    ...
    fseek(fp, 4,SEEK_CUR); /* added by learningx skip the biSize member.*/
    ...
    colornum = (int)wTemp;
    fseek(fp, 4, SEEK_CUR); /* added by learningx skip the biCompression members.*/
    ...
    imagesize += align_32_bits(w>>3) * h;
    fseek(fp, 4, SEEK_CUR);//added by learningx
    //fseek(fp, sizeof(DWORD), SEEK_CUR);

    /* skip the rest members and the color table.*/
    fseek(fp, 4*4, SEEK_CUR);//added by learningx
