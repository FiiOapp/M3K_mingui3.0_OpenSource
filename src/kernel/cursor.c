...
HCURSOR GUIAPI LoadCursorFromFile(const char* filename)
{
    ...
    fseek(fp, 4, SEEK_CUR); /* skip the biSize member. added by learningx*/
    ...
    fseek(fp, 4, SEEK_CUR); /* skip the biCompression members. added by learningx*/
    imagesize = MGUI_ReadLE32FP (fp);

    /* added by learningx skip the rest members and the color table. */
